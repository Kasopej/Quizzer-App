...
if (
  ...
  //Checks if categories have been successfully saved in app. Returns boolean
...
function checkAndValidateQuantityInput() {
  ...
  quizzerDataOperation
    .qtyOfQuestionsAvailable(
      categoryOptionElements[selectCategoryElement.selectedIndex].value,
      difficultyOptionElements[selectDifficultyElement.selectedIndex].value
    )
    .then((quantity) => {
...
function save_UI_Config_Entries(event) {
  //On form submit, save form selections to configData
  const selectedCategoryOptionElement =
    categoryOptionElements[selectCategoryElement.selectedIndex];
  const selectedDifficultyOptionElement =
    difficultyOptionElements[selectDifficultyElement.selectedIndex];
  const selectedTypeOptionElement =
    typeOptionElements[selectTypeElement.selectedIndex];
  const selectedTimingOptionElement =
    timingOptionElements[selectTimingElement.selectedIndex];
...
function processEmailEntries(candidatesEmails) {
  //Validate emails and print links (if emails valid). Copy configuration data to each email link
